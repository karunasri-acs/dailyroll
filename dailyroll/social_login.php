...
$log->event_log(json_encode($data),basename(__FILE__),'d');
	if($data !=''){
if (isset($data['socialname'])&& isset($data['socialid'])) {
	$email=$data['socialemail'];
	$log->event_log($email,basename(__FILE__),'d');
	$name=$data['socialname'];
	$log->event_log($name,basename(__FILE__),'d');
	$id=$data['socialid'];
	$log->event_log($id,basename(__FILE__),'d');
	...
	if($email !='' ){
		...
		$log->event_log(json_encode($emailcheck),basename(__FILE__),'d');
		if($emailcheck == 'null'){
				$uuid = uniqid('', true);
				$sql="INSERT INTO `users`( `unique_id`, `name`, `email`,`created_at`,`userStatus`,`socialloginid`,`phone`) VALUES ('$uuid','$name','$email',NOW(),'Y','$id','$laststring')";
				$log->event_log($sql,basename(__FILE__),'d');
				...
				$log->event_log(json_encode($response),basename(__FILE__),'d');
		...
		$stmt4->execute();
			...
			$response["email"] = $emailcheck["email"];
