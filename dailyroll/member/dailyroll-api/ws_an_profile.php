...
//$log->$log->event_log('root file beginning ','d'); 
   ...
   /* $name="hii";
	...
	if($count==0){
		...
		$stmt = $user_home->runQuery($sql);
		$stmt->execute();
		$sql="INSERT INTO `profile`(`user_id`, `name`, `lastname`, `email`, `address`, `phone`, `country`, `photo`) 
					VALUES ('$id','$name','$lname','$email','$address','$phone','$country','$filepath')";
					$log->event_log($sql,'d');
		$stmt = $user_home->runQuery($sql);
		$stmt->execute();
		...
		$stmt = $user_home->runQuery($sql);
		$stmt->execute();
		$sql1="UPDATE `profile` SET `name`='$name',`lastname`='$lname',`email`='$email',`address`='$address',`phone`='$phone',`country`='$country',`photo`='$filepath' WHERE `user_id`='$id'";
