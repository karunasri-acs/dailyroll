 ...
 if(isset($_POST['uid']) && isset($_POST['email']))
{
	...
	$user_home->redirect('m_charts.php');
...
<!-- datepicker -->
<script src="public/bootstrap-datepicker/dist/js/bootstrap-datepicker.min.js"></script>
<!-- Bootstrap WYSIHTML5 -->
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script> 
 ...
 function drawChart() {
    ...
    [ 'Category', 'Amount',],
		  <?php
			$sql1="select subcat_id,SUM(amount) as number from `expenses` where capture_id ='$unid'";
			
			 if($category =='all' AND $accid ==''){
				$sql1=$sql1;
				}
		  	else if($category =='all' AND $accid !=''){
				$sql1=$sql1."AND  `account_id`='$accid'";
				}
			else if($category !='' AND $accid !=''){
				$sql1=$sql1."AND  `account_id`='$accid' AND `cat_id`='$category'";
				}
				$sql1=$sql1."GROUP BY `subcat_id`";
		    $stmt2=$user_home->runQuery($sql1);
			$stmt2->execute();
		while($row2=$stmt2->fetch(PDO::FETCH_ASSOC)){
			$category_name=$user_home->get_subcategory($row2['subcat_id']);
			$category=$category_name;
			echo "['".$category."',".$row2['number']."],";
		}
		?>
		]);
  ...
  width: 450,
  height: 440,
  colors: ['SlateBlue','red','SlateBlue','red','green','red',],
  is3D: true
});
